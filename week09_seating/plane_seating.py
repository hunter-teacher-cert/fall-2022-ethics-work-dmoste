...
def seat_economy(plane,economy_sold,name,number):
    """
    ...
    rows = len(plane)
    cols = len(plane[0])
    open_seats = ["win", "avail"]

    row = 0
    col = 0
    seats_found = 0
    while seats_found < number:
        if row < rows:
            if plane[row][col] in open_seats:
                seats_found += 1
                for i in range(1, number + 1):
                    if i < cols and plane[row][col+i] in open_seats:
                        seats_found += 1
        
        if seats_found < number:
            seats_found = 0
            row += 1
            col += 1
        else:
            for i in range(0, number):
                plane[row][col+i] = name
    return plane
...
def fill_plane(plane):
    ...
    by_size = list(economy_sold.items())
    by_size.sort(key=lambda a: a[1], reverse = True)
    for group in by_size:
        plane = seat_economy(plane,economy_sold, group[0], group[1])
